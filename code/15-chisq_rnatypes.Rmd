...
starCounts <- read.csv("../output/STAR_count_data.csv", header=TRUE, row.names=1)
head(starCounts)
...
```{r}
#normalizing the data using ratio of means (size factors)
...
stacked_bar <- ggplot(pie_long, aes(x = group, y = value, fill = factor(Type))) +
  geom_bar(stat = "identity", width = 0.5, position = "fill") +  # Stacked bar chart
  scale_y_continuous(labels = scales::percent) +  # Show percentage
  theme_minimal() +
  scale_x_discrete(labels=c("unprimed eggs", "primed eggs")) +
  labs(x = "", y = "Proportion of Reads", fill = "Type") +
  ggtitle("Proportion of Reads by Gene Type")
